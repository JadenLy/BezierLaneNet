...
class Resize(object):
    ...
    def parse_resize(image, target, size_image, size_label, ori_size, ignore_x):
        ...
        if target is None or isinstance(target, str):
            ...
            if 'keypoints' in target.keys():
                target['keypoints'] = resize(target['keypoints'], ori_size, size_label, ignore_x)
    ...
    def __call__(self, image, target=None):
        w_ori, h_ori = F.get_image_size(image)
...
class Crop(object):
    ...
    def parse_crop(image, target, top, left, height, width, ignore_x):
        ...
        if target is None or isinstance(target, str):
            ...
            if 'keypoints' in target.keys():
                target['keypoints'] = crop(target['keypoints'], top, left, height, width, ignore_x)
            # if 'padding_mask' in target.keys():
...
class RandomTranslation(object):
    ...
    def __call__(self, image, target):
        ...
        if target is None or isinstance(target, str):
            ...
            if 'keypoints' in target.keys():
                target['keypoints'] = translate(target['keypoints'], tw - j, th - i, th, tw, self.ignore_x)
            # if 'padding_mask' in target.keys():
            #     pass
            if 'segmentation_mask' in target.keys():
    ...
    def __call__(self, image, target=None):
        ...
        if t < self.flip_prob:
            ...
            if target is None or isinstance(target, str):
                ...
                if 'keypoints' in target.keys():
                    target['keypoints'] = hflip(target['keypoints'],
                                                     F.get_image_size(image)[0] / 2, self.ignore_x)
                # if 'padding_mask' in target.keys():
    ...
    def __call__(self, image, target=None):
        ...
        if self.normalize_target and not (target is None or isinstance(target, str)):
            if isinstance(target, dict):
                if 'keypoints' in target.keys():
                    w, h = F.get_image_size(image)
                    target['keypoints'] = normalize(target['keypoints'], h, w, self.ignore_x)
...
class MatchSize(object):
    ...
    def __call__(self, image, target):
        wi, hi = F.get_image_size(image)
        wl, hl = F.get_image_size(target)
        if hi == hl and wi == wl:
            return image, target
...
class RandomRotation(object):
    ...
    def __call__(self, image, target):
        ...
        if isinstance(target, dict):  # To keep BC
            if 'keypoints' in target.keys():
                w, h = F.get_image_size(image)
                target['keypoints'] = rotate(target['keypoints'], angle, h, w, self.ignore_x)
            # if 'padding_mask' in target.keys():
...
class RandomAffine(torch.nn.Module):
    ...
    def forward(self, image, target):
        ...
        image = F.affine(image, *ret, interpolation=Image.LINEAR, fill=0)

        if target is None or isinstance(target, str):
            return image, target
        elif isinstance(target, dict):  # To keep BC
            if 'keypoints' in target.keys():
                target['keypoints'] = affine(target['keypoints'], *ret,
                                                  height=img_size[1], width=img_size[0], ignore_x=self.ignore_x)
            # if 'padding_mask' in target.keys():
            #     pass
            if 'segmentation_mask' in target.keys():
                target['segmentation_mask'] = F.affine(target['segmentation_mask'], *ret,
                                                       resample=Image.NEAREST, fill=255)
        else:
            target = F.affine(target, *ret, resample=Image.NEAREST, fill=255)
